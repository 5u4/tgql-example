...
Use [graphql playground](https://tgql-example.herokuapp.com/) to test it out.
...
query {
  people(
    ...
    name
    born
    movies(pagination: {page: 1, perPage: 1}) {
      title
      personRelation
    }
  }
...
query {
  movies(
    pagination: {page: 1, perPage: 100}
    filters: {titleContains: "", releasedAfter: 2000, operator: OR}
  ) {
    identity
    title
    released
    tagline
    people(pagination: {page: 1, perPage: 10}) {
      name
      roles
    }
  }
...
Database contains Neo4J person movie example data.
